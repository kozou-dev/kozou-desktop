...
// hand to an AI agent. Same pure functions, same serialization as the MCP
// server (JSON.stringify(payload, null, 2)) — nothing paraphrased.
//
// Runs inside the worker so the renderer never depends on @kozou/mcp.

import type { SchemaContext } from '@kozou/core';
import { describeFunctions, describeTable, describeView, getConceptContext } from '@kozou/mcp';
import type { AiViews } from '../shared/types.js';

/** Serialize exactly as the MCP server does for tool results. */

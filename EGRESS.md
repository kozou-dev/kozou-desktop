...
**The bridge now ships (2026-08-07) — and that is all that can be said.** This
build contains the relay described in item 16, and its behaviour is tested
against the real local MCP server with the official MCP client. What is *not*
established: that Claude Desktop spawns it and holds a conversation over it. No
connection snippet for it is offered anywhere in the UI yet, and no third-party
client has been pointed at it. Until a run like the Claude Code one above exists
for Claude Desktop, "Kozou works in Claude Desktop" is not a claim this project
has earned.

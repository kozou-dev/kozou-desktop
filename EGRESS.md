...
| 16 | The stdio bridge relays to loopback, holds nothing, listens for nothing, and starts nothing | An AI client that speaks only stdio (Claude Desktop) cannot reach a loopback URL, so this build ships a **relay**: a small process the *client* spawns, which forwards newline-delimited JSON-RPC to a server this app is already running. It is not an app process — the app never spawns, tracks or restarts it — and it changes nothing about the server, which stays exactly as item 10 describes. Four properties, each with its own enforcement. **(a) Destination**: after WHATWG normalization the host must equal `127.0.0.1` or `::1` — not a range (`127.0.0.2` is refused), not a name (`localhost` is refused, since its resolution is not ours to decide), not a lookalike (`127.0.0.1.evil.example`), not an IPv4-mapped IPv6 form, not a userinfo forgery (`http://127.0.0.1@evil.example/`); credentials in the target are refused outright, and redirects are never followed. This is the one property that matters beyond hygiene: a generic stdio→HTTP relay with an editable destination is how "a remote MCP server without OAuth" would come back. **(b) Holds nothing**: its import graph is `src/` files and Node builtins only — no database driver, no `@kozou/*`, no MCP SDK — one module may touch the filesystem, and no bridge source may name the profile store. **(c) Listens for nothing**: observed rather than argued — the packaged bridge is run as a real process, driven through a whole session, and its own socket table is read (zero listening sockets), with a positive control that a process which *is* listening gets caught by the same detector. A static scan cannot make this claim: the bridge legitimately needs outbound HTTP, so its imports do not distinguish client from server. **(d) Starts nothing**: with no server published it prints why and exits non-zero; it never launches the app. **New local file, stated because it is new**: `<userData>/mcp-locators/<id>.json`, mode `0600`, written when a profile's server starts and removed when it stops (with a sweep at launch, so a crash leaves none). It carries an opaque id, the port, the capability path and a generation — never a URL, a user name or a password blob — and it exists so the capability path stays out of other applications' config files. Not egress; a local surface all the same. **What is not verified**: that Claude Desktop actually spawns it. No config for it is offered to users in this build, and no third-party client has driven it | unit + process tests (`test/bridgeLoopback.test.ts`: 12 refusals and 2 normalizing acceptances; `test/mcpLocator.test.ts`: a locator carrying any unknown field is refused; `test/bridgeRelay.test.ts`: session handling, 202 silence, no redirect following; `test/bridgeProcess.test.ts`: the live process's sockets, and the refusals above as process behaviour) + `scripts/check-treeshake.mjs` and `scripts/check-egress-static.mjs` (import graph, single fs module, no server construction) + integration (`test/bridge.integration.test.ts`: the SDK's own client, over the bridge and straight at the server, returning the same tool list and the same tool result against a real database) |

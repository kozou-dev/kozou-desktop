...
grant says. As of this build the UI reaches the **read** half of that path: a
profile card grants row browsing (native approval) and a Data tab lists rows.
The mutation channels are still plumbing — nothing in the shipped UI calls
them, and no screen asks for a `readwrite` grant.

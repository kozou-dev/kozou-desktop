...
**Claude Desktop, through the bridge: attempted and reached (2026-08-07).** The
relay described in item 16 ships in this build. Given a **hand-written**
`mcpServers` entry — `command` = the packaged app binary, `args` = the bridge
script inside `app.asar` plus `--id <locator id>`, `env` =
...
**What that run does not establish**, kept separate on purpose: no `tools/call`
appears in the desktop MCP log — the tool call above came from the Claude Code
surface of the same application, so *Claude Desktop's chat calling a tool* is
still unobserved. The entry was written by hand; the app offers nothing that
produces it. And this is one run, on one machine, against one client version.

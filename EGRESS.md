...
**The produced entry, and what is still missing (2026-08-09).** The config panel
now produces an entry, and one was pasted by hand into the same client
(1.26832.0): it spawned the packaged binary running the script from inside
`app.asar` — process tree parent `Claude.app` — and that client's MCP log records
the handshake and `tools/list` answered. A tool call over it returned the
database's compiled semantics. Two things this does **not** carry. The call came
from the same application's **Claude Code** surface rather than its chat, so the
chat half is still evidenced only by the hand-written run above; the two halves
have not been demonstrated on one entry. And this run used a **locally built**
`.app`, unsigned and never quarantined — §"Distribution" applies unchanged.

...
| 14 | A row is written only where it can be named, and only what was answered for | the editor is offered for a relation this app can address a single row of, and nowhere else: not for a view (kozou answers a write to one with a 405), not for a table without a primary key, and not for a row whose key cannot be expressed in the item address — a component of a composite key containing the separator it is split on would otherwise name a *different* row. Every field answers one of three ways — leave it to the database, send a value, or send null — and the payload carries only what was answered for: an insert omits what it was not given (so a generated key is generated and a DEFAULT applies), and an update sends only the fields that were edited. That last rule is what keeps a value this app cannot represent faithfully (a byte array; a json document re-serialized to fill a control) from being written back in whatever shape it survived the trip in — and such a field is locked in the form rather than seeded with a rendering of the value. An editor also re-reads its row through the single-row `get` path before opening, so a value the browse pane had to cut is never saved back cut (item 13). Independently, the row-data pool hands date, time and interval values over as PostgreSQL's own **text**: the driver's parsers for that family are not round-trippable, and a `date` parsed into an instant names the previous day under any clock behind UTC — measured, `2026-08-01` arriving as `2026-07-31T15:00:00.000Z` | unit tests (`test/rowForm.test.ts`: the payload rules, the lock rules, and the ids that are refused) + integration (`test/data.integration.test.ts`: a real `date` read back as itself and accepted back unchanged) + e2e (`e2e/crud.spec.ts`: a real insert/edit/delete round trip; no write control on a view or on a table with no primary key; a unique violation shown as the fixed sentence without the value that collided) |

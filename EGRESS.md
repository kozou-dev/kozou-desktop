...
exception is the opt-in **local MCP hub**, which takes two acts and is off
without both: the user allows profiles to serve (off by default) and then starts
a particular profile's server. Once started, the app listens on **127.0.0.1
...
grant says. As of this build the UI reaches both halves of that path: a profile's
own row — the bar above the map while it is selected, or its card in the
all-databases view — grants row browsing (native approval) and a Data tab lists
rows, and a second grant — its own level, its own approval — adds insert, edit and delete
...
| 3 | No outbound app-code network calls | no `fetch`/XHR/WebSocket, no Electron `net`/`session.fetch`, and no `node:http(s)`/`net`/`tls`/`dgram` import anywhere under `src/` — with **one carve-out**: the stdio bridge's single HTTP client module (`src/bridge/httpClient.ts`), whose whole job is a request to this machine's own loopback address (item 16). Every other file under `src/bridge/` is held to the same no-network-module rule as the rest of `src/`, and the app processes themselves are unchanged: the local MCP server gets its listener from `@kozou/mcp`, not from a direct import | same (token + import scan; the carve-out names one file and fails if that file stops existing) |
...
| 16 | The stdio bridge relays to loopback, carries no database state, listens for nothing, and starts nothing | An AI client that speaks only stdio (Claude Desktop) cannot reach a loopback URL, so this build ships a **relay**: a small process the *client* spawns, which forwards newline-delimited JSON-RPC to a server this app is already running. It is not an app process — the app never spawns, tracks or restarts it — and it changes nothing about the server, which stays exactly as item 10 describes. Four properties, each with its own enforcement. **(a) Destination**: after WHATWG normalization the host must equal `127.0.0.1` or `::1` — not a range (`127.0.0.2` is refused), not a name (`localhost` is refused, since its resolution is not ours to decide), not a lookalike (`127.0.0.1.evil.example`), not an IPv4-mapped IPv6 form, not a userinfo forgery (`http://127.0.0.1@evil.example/`); credentials in the target are refused outright, and redirects are never followed. This is the one property that matters beyond hygiene: a generic stdio→HTTP relay with an editable destination is how "a remote MCP server without OAuth" would come back. **(b) Carries no database state**: it holds the HTTP session id, the message in flight, the deadline currently in force and the set of requests it has open, and beyond that its import graph is `src/` files and Node builtins only — no database driver, no `@kozou/*`, no MCP SDK — one module may touch the filesystem, and no bridge source may name the profile store. **(c) Listens for nothing**: observed rather than argued — the bridge, bundled as it ships, is run as a real process, driven through a whole session, and *that process's* socket table is read (zero listening sockets), with a positive control that a process which *is* listening gets caught by the same detector. Scope of that run: the test runner's Node executes the bundle; the packaged `.app` binary running the script from inside its own asar was exercised by hand instead (2026-08-07, full session relayed, exit 0), not in CI. A static scan cannot make the claim at all: the bridge legitimately needs outbound HTTP, so its imports do not distinguish client from server. **(d) Starts nothing**: with no server published it never launches the app — it stays where it is and answers every request with a JSON-RPC error carrying the reason until the client closes, then ends non-zero. Observed the way (c) is: the bridge's own child processes are read while it runs, on the healthy path and the no-locator path alike, with a positive control that a process which *does* start one is caught by the same detector. That detector is a point-in-time sample, so a child that has already exited leaves no trace in it. **New local file, stated because it is new**: `<userData>/mcp-locators/<id>.json`, mode `0600`, written when a profile's server is confirmed listening and withdrawn whenever the app stops serving it — including when the worker ignores the kill, because the app's decision to stop is what the file tracks. Two gaps in "present only while serving", stated rather than implied: a `SIGKILL`ed main process leaves one behind until the **next launch**, whose sweep clears it, and a failed unlink is swallowed. A stale locator normally makes the bridge fail to connect — but if another local process has taken that port since, the bridge relays to it, so "stale" is not the same as "inert". BR-1 bounds what that can be to this machine's own loopback and nothing beyond it. The file carries an opaque id, the port, the capability path and a generation — never a URL, a user name or a password blob — and it exists so the capability path stays out of other applications' config files. Not egress; a local surface all the same. **Verified once, by hand (2026-08-07)**: Claude Desktop 1.26832.0, given a hand-written `mcpServers` entry, spawned the packaged app binary running this script from inside `app.asar` and completed the handshake — its own log records `initialize`, `notifications/initialized` (no reply, as intended) and `tools/list`, and a `list_tables` call over the same entry returned the database's compiled semantics. Later in that same connection two `tools/call` requests appear in that log, each answered with a result carrying one content block; the log names neither the tool nor the surface that asked, and the attribution to Claude Desktop's own chat comes from watching that chat answer with the database's semantics at those timestamps. **Verified a second time, with the entry this app produces, and used from the chat (2026-08-09)**: the config panel's entry, pasted by hand into the same client (1.26832.0), spawned the packaged binary running the asar-internal script — the process tree shows `Claude.app` as the parent — and the client's own MCP log records `Server started and connected successfully`, `initialize` answered, `notifications/initialized` with no reply, and `tools/list` answered. Eighteen minutes later that log carries two `tools/call` requests, each answered with a result carrying one content block, and the chat replied with the database's compiled semantics at those timestamps. **The log alone does not support the attribution** — it names neither the tool nor the surface that asked — so the claim rests on the log together with what the chat returned; one detail strengthens it beyond the 2026-08-07 run, which rested on the same two legs: the chat's answer listed the same relations, by name, that a call over the same entry had returned independently. **Still bounded**: one machine, one client version, one run, on a locally built unsigned app. Incidentally it settles a comparison this document could not make before: the hand-written entry was still in that client's configuration file, and it matched the produced one in `command`, in the asar-internal script path, in `--id` and in `env` — differing only in the server name. **And property (d) was observed through a real client for the first time**: with the app stopped, a call over the same entry came back as a JSON-RPC error naming a refused connection to the loopback port, having started nothing — until now that behaviour had only ever been driven as a process test. **Scope of the checks**: they constrain this repository's source — an import graph, one filesystem module, one read call, one asked-for path, no server-construction tokens. They are not an OS-level sandbox: the base directory can come from the environment and a symlink at the locator's own name would be followed, both of which need write access to the user's own files | unit + process tests (`test/bridgeLoopback.test.ts`: 12 refusals and 2 normalizing acceptances; `test/mcpLocator.test.ts`: a locator carrying any unknown field is refused; `test/bridgeRelay.test.ts`: session handling, 202 silence, no redirect following; `test/bridgeProcess.test.ts`: the live process's sockets and its child processes, a broken stdout reported once and left, and the unreachable-server answers above as process behaviour) + `scripts/check-treeshake.mjs` and `scripts/check-egress-static.mjs` (import graph, single fs module, no server construction) + integration (`test/bridge.integration.test.ts`: the SDK's own client, over the bridge and straight at the server, returning the same tool list and the same tool result against a real database) |
...
URL using the command the **AI client config** panel produced, against
...
the known workaround. Recorded as a negative result rather than a gap: the app
no longer offers that path.

**Claude Desktop, through the bridge: reached, and used from its own chat
(2026-08-07).** The relay described in item 16 ships in this build. Given a
**hand-written** `mcpServers` entry — `command` = the packaged app binary,
`args` = the bridge script inside `app.asar` plus `--id <locator id>`, `env` =
`ELECTRON_RUN_AS_NODE=1` — Claude Desktop 1.26832.0 spawned it and connected.
Its own MCP log (`~/Library/Logs/Claude/mcp-server-*.log`) records "Server
started and connected successfully", then `initialize`, then
`notifications/initialized` with no reply, then `tools/list` answered; the
process tree confirms the parent is `Claude.app` and the child is the packaged
binary running the asar-internal script. A `list_tables` call over the same
entry returned the database's compiled semantics.

**Then the desktop's own chat used it.** Nearly six hours into that same
connection — one spawn, no reconnect in the log — two `tools/call` requests
(`id=2`, `id=3`) appear there, each answered by a result carrying one content
block. That is all the log carries: it names neither the tool nor the surface
that asked, so on its own it cannot say the chat was the caller. The other half
was watched on screen at those timestamps — the chat answering with the
database's compiled semantics, which it had no other route to. The claim is the
two together; the log alone would not support it.

**What that run does not establish**, kept separate on purpose: the entry was
written by hand, so it showed nothing about a user reaching this from the app
itself. And it is one run, on one machine, against one client version.

**And the relay has changed since that run**, which is the part easiest to read
past: request deadlines, cancellation when the client leaves, a broken-pipe
path and an event-stream refusal all landed afterwards. What the run verified
is the shape of the path — this client spawns this script from inside the asar
and speaks MCP through it — not this exact code. The hand-verified relay and
the shipped relay are no longer the same bytes.

**The produced entry, end to end (2026-08-09).** The config panel now produces an
entry, and one was pasted by hand into the same client (1.26832.0): it spawned
the packaged binary running the script from inside `app.asar` — process tree
parent `Claude.app` — and that client's MCP log records the handshake and
`tools/list` answered, then two `tools/call` requests eighteen minutes later,
each answered with one content block, with the chat replying in the database's
compiled semantics at those timestamps. The log names neither tool nor calling
surface, so that attribution is the log plus what the chat returned — the same
two-legged claim as the 2026-08-07 run, with one leg firmer: the relations the
chat named matched, one for one, what a call over the same entry returned
independently.

What this run does **not** carry: it used a **locally built** `.app`, unsigned
and never quarantined, so §"Distribution" applies unchanged; and it remains one
machine, one client version, one run.

What it does settle is a check no CI job performs: `pnpm dist` is not part of the
workflow, so nothing automated ever resolves the bridge path inside a packaged
bundle. That resolution was exercised here for the first time on a produced
entry. A launcher that resolved against the working directory would pass every
check in this repository and fail only in a packaged app — measured, not
supposed: making that substitution leaves the whole suite green.

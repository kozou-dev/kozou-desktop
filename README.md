...
**Not sure whether you want this or a Kozou server?** This app is the one you look at — several databases' semantics in one interactive view. A server is what other things reach: a database your machine cannot open a socket to, a client that only accepts a URL it can reach over the network, a second person, or an agent that should execute rather than only read. [The desktop app, or a server?](https://kozou.org/concepts/desktop-or-server/) is the decision in full.
